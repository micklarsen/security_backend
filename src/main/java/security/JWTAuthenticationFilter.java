...
public class JWTAuthenticationFilter implements ContainerRequestFilter {
 ...
 public void filter(ContainerRequestContext request) throws IOException {
   if (isSecuredResource()) {
     ...
     if (token == null) {
       request.abortWith(errorhandling.NotFoundExceptionMapper.makeErrRes("Not authenticated - do login", 403));
       return;
     }
     try {
       ...
       request.abortWith(errorhandling.NotFoundExceptionMapper.makeErrRes("Token not valid (timed out?)", 403));
     }
   }
 }
 ...
 private UserPrincipal getUserPrincipalFromTokenIfValid(String token)
   ...
   if (signedJWT.verify(verifier)) {
     ...
     String useremail = signedJWT.getJWTClaimsSet().getClaim("useremail").toString();

     String[] rolesArray = roles.split(",");
     
     return new UserPrincipal(username, useremail, rolesArray);
